...
类类型对象及其指针、引用的转换：
  ...
  //set print object on ”设为on，才能通过p bp ,p *bp打印父类指针bp和*bp的实际类型(动态类型)
    1.若父类一个虚函数都没有,指向子类对象的父类指针bp的静态类型和动态类型都是父类指针类型(Base*)
      也就是说父类指针bp实际类型(动态类型)是编译期确定的、就是Base*类型.
      也同时表明*bp是Base类对象。
      也就是说bp->虚函数的调用是静态绑定，编译期就确定是Base的该虚函数。
      1.1如果子类也一个虚函数没有.
          那此时子类对象d也就没有虚函数表指针，此时p bp打印的地址和p &d打印的一样
      1.2如果子类有至少一个虚函数
          那此时，子类对象d有虚函数表指针，但是p bp打印的地址是&d+8(虽然Base×bp =&d)
    2.若父类有至少一个虚函数，无论子类是否有相应的虚函数overide，还是子类压根就没虚函数，
      ...
      也就是说bp->虚函数的调用是动态绑定，但不一定就是*bp实际类型的哪个版本。
      动态绑定是根据bp指针的虚函数指针指向的虚函数表去选择哪一个版本的该虚函数
      如果bp指针指向子对象，但该子类型没有该虚函数或者压根就没有任意的虚函数，那即便bp指向子类
      对象，调用该虚函数时也是选择的是父类的该虚函数。
